...
if($action=="") {
	...
	$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads ".$getsquad." WHERE displayed='1' ORDER BY sort");
	while($ds=mysql_fetch_array($ergebnis)) {
		...
		if($ds['icon']) $icon='images/squadicons/'.$ds['icon'].'';
		else $icon='';
		...
		eval("\$squads_head = \"".gettemplate("squadfull_head")."\";");
		echo $squads_head;
		...
		while($dm=mysql_fetch_array($member)) {
			...
			$nickname = '<a href="user/'.$dm['userID'].'/">'.strip_tags(stripslashes($dm['nickname'])).'</a>';
			...
			$res = safe_query("SELECT birthday, DATE_FORMAT(FROM_DAYS(TO_DAYS(NOW()) - TO_DAYS(birthday)), '%y') 'age' FROM ".PREFIX."user WHERE userID = '".$profilid."'");
			...
			eval ("\$squads_content = \"".gettemplate("squadfull_content")."\";");
			echo $squads_content;
			$i++;
		}
		eval ("\$squads_foot = \"".gettemplate("squadfull_foot")."\";");
		echo $squads_foot;
	}
}
