...
function forum_stats() {
	...
	if($user) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			if($n>1) $user_names .= ', <a href="user/'.$ds['userID'].'">'.$nickname.'</a>';
			else $user_names = '<a href="user/'.$ds['userID'].'">'.$nickname.'</a>';
			$n++;
		}
	}
...
function boardmain() {
	...
	while($dk=mysql_fetch_array($kath)) {
		$kathname = "<a href='forum/cat/".$dk['catID']."'>".$dk['name']."</a>";
		if($dk['info']) $info=$dk['info'];
		else $info='';

	if ($dk['catico_grp']=="1") 	$catico='images/games/'.$dk['catico'].'.gif';
	elseif ($dk['catico_grp']=="2") $catico='images/flags/'.$dk['catico'].'.gif';
	elseif ($dk['catico_grp']=="3") $catico='images/smileys/'.$dk['catico'].'';
	elseif ($dk['catico_grp']=="4") $catico='images/forumico/'.$dk['catico'].'.jpg';
	else $catico='images/forumico/no_banner.jpg';
		...
		while($db=mysql_fetch_array($boards)) {
			...
			$anzposts=$db['posts'];
			$boardname = $db['name'];
			$boardname ='<a href="forum/board/'.$board.'">'.$boardname.'</a>';

			if($db['info']) $boardinfo=$db['info'];
			else $boardinfo='';
			
			if ($db['boardico']=="") $boardico='<img src="images/forumico/no_icon.jpg"/>';
			else $boardico='<img src="images/forumico/'.$db['boardico'].'.jpg"/>';	
			...
			else $boardico='<img src="images/forumico/no_icon.jpg"/>';					
			...
			while($lp = mysql_fetch_assoc($q)) {
				...
				if($n == 1) {
					...
					$poster='<a href="user/'.$lp['lastposter'].'">'.getnickname($lp['lastposter']).'</a>';
			...
			if($found) $icon='<img src="images/icons/boardicons/'.$writer.'on.png" alt="'.$_language->module['new_posts'].'" />';
			else $icon='<img src="images/icons/boardicons/'.$writer.'off.png" alt="'.$_language->module['no_new_posts'].'" />';

			if($topictitle == ''){
                    $noentry = ''.$_language->module['no_entry'].'';
               }
               else{
                     $noentry = '<a href="'.$postlink.'">'.$topictitle.'</a> '.$_language->module['by'].' '.$poster.'';
               }
	...
	while($db=mysql_fetch_array($boards)) {
		...
		$boardname = $db['name'];
		$boardname='<a href="index.php?site=forum&amp;board='.$db['boardID'].'">'.$boardname.'</a>';
...
function showboard($board) {
	...
	$page_link = '';
	if($pages>1) $page_link = makepagelink("forum/board/$board/", $page, $pages);
	...
	if($loggedin) {
		$mark='';
		if($writer) $actions.='<a class="forum_button" href="index.php?site=forum&amp;addtopic=true&amp;board='.$board.'">New Topic</a>';
	} else $mark='';
	...
	if($anztopics) {
		...
		while($dt=mysql_fetch_array($topics)) {
			...
			if($dt['sticky']) {
				$onicon = '<img src="sm/forum/newsticky.png" alt="'.$_language->module['sticky'].'" />';
				$officon = '<img src="sm/forum/sticky.png" alt="'.$_language->module['sticky'].'" />';
				$onhoticon = '<img src="sm/forum/newsticky.png" alt="'.$_language->module['sticky'].'" />';
				$offhoticon = '<img src="sm/forum/sticky.png" alt="'.$_language->module['sticky'].'" />';
			}
			else {
				$onicon = '<img src="sm/forum/newfolder.png" alt="'.$_language->module['new_posts'].'" />';
				$officon = '<img src="sm/forum/folder.png" alt="no '.$_language->module['new_posts'].'" />';
				$onhoticon = '<img src="sm/forum/newhotfolder.png" alt="'.$_language->module['new_posts'].' ['.$_language->module['popular'].']" />';
				$offhoticon = '<img src="sm/forum/hotfolder.png" alt="no '.$_language->module['new_posts'].' ['.$_language->module['popular'].']" />';
			}

			if($dt['closed']) $folder='<img src="sm/forum/lockfolder.png" alt="'.$_language->module['closed'].'" />';
			elseif($dt['moveID']) $folder='<img src="sm/forum/pfeil.png" alt="'.$_language->module['moved'].'" />';
			elseif($userID) {

				$is_unread = mysql_num_rows(safe_query("SELECT userID FROM ".PREFIX."user WHERE topics LIKE '%|".$dt['topicID']."|%' AND userID='".$userID."'"));

				if($is_unread) {
					if($dt['replys']>10 || $dt['views']>150) $folder=$onhoticon;
					else $folder=$onicon;
				}
				else {
					if($dt['replys']>10 || $dt['views']>150) $folder=$offhoticon;
					else $folder=$officon;
				}
			}
			else {
			...
			$poster='<a href="user/'.$dt['userID'].'/">'.getnickname($dt['userID']).'</a>';
			...
			if($dt['moveID']) { // MOVED TOPIC
				...
				$link='<a href="index.php?site=forum_topic&amp;topic='.$dt['moveID'].'">'.$_language->module['moved'].': '.$topictitle.'</a>';
				...
				$link='<a href="forum/topic/'.$dt['topicID'].'">'.$topictitle.'</a>';
			}
	...
	if(!$loggedin) echo '<div id="forum_register"><div class="attention">You must login or <a href="register/">register</a> to use the forum.</div></div>';

	unset($page_link);
}

if(isset($_POST['submit']) || isset($_POST['movetopic']) || isset($_POST['deletetopics']) || isset($_GET['addtopic']) || isset($_POST['addtopic']) || (isset($_GET['action']) and $_GET['action'] == "admin-action") || isset($_POST['admaction'])) {
...
if(isset($_POST['topicID'])) {
	if(is_array($_POST['topicID'])) {
		$topicID = $_POST['topicID'];
	} else {
		$topicID = array($_POST['topicID']);
	}
} else $topicID = array();
if(!isforumadmin($userID) and !ismoderator($userID, $board)) die($_language->module['no_access']);
foreach($topicID as $id) {
	safe_query("UPDATE ".PREFIX."forum_topics SET closed='1' WHERE topicID='$id' ");
}
//FORUM TOPIC ADDON END
...
if(!isforumadmin($userID) and !ismoderator($userID, $board)) die($_language->module['no_access']);
foreach($topicID as $id) {
	safe_query("UPDATE ".PREFIX."forum_topics SET closed='0' WHERE topicID='$id' ");
}
//FORUM TOPIC ADDON END
		header("Location: index.php?site=forum&board=$board");
	}
...
if(!isforumadmin($userID) and !ismoderator($userID, $board)) die($_language->module['no_access']);
foreach($topicID as $id) {
	safe_query("UPDATE ".PREFIX."forum_topics SET sticky='1' WHERE topicID='$id' ");
}
//FORUM TOPIC END
		header("Location: index.php?site=forum&board=$board");
	}
...
if(!isforumadmin($userID) and !ismoderator($userID, $board)) die($_language->module['no_access']);
foreach($topicID as $id) {
	safe_query("UPDATE ".PREFIX."forum_topics SET sticky='0' WHERE topicID='$id' ");
}
		header("Location: index.php?site=forum&board=$board");
	}
	...
	if(isset($_POST['topics'])) $topicID =$_POST['topics'];
	else $topicID = array();
	...
	foreach($topicID as $id) {
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."forum_topics WHERE topicID='$id'");
		$ds=mysql_fetch_array($ergebnis);

		if(isset($_POST['movelink']) and $ds['boardID'] != $toboard) safe_query("INSERT INTO ".PREFIX."forum_topics (boardID, icon, userID, date, topic, lastdate, lastposter, replys, views, closed, moveID) values ('".$ds['boardID']."', '', '".$ds['userID']."', '".$ds['date']."', '".addslashes($ds['topic'])."', '".$ds['lastdate']."', '', '', '', '', '$id') ");
	...
	elseif($_POST['admaction']=="movetopic") {
		...
		} else header("Location: index.php?site=forum&board=".$_POST['board']);		
		$_language->read_module('forum_admin_options', true);
		foreach($topicID as $id) {
		$title = mysql_fetch_row(safe_query("SELECT topic FROM ".PREFIX."forum_topics WHERE topicID='".$id."'"));
		$topics .= '<option selected="selected" value="'.$id.'">'.$title[0].'</option>';
		}
		//FORUM TOPIC ADDON END
	...
	elseif(isset($_REQUEST['addtopic'])) {
		...
		eval ("\$forum_head = \"".gettemplate("forum_newtopic_head")."\";");
		echo $forum_head;
		...
		if($loggedin) {
			if(isset($_POST['preview'])) {
				...
				if(getemail($userID) and !getemailhide($userID)) $email = '<a href="mailto:'.mail_protect(getemail($userID)).'"><img src="images/icons/email.png" border="0" alt="email" /></a>';
				else $email='';
				
				$pm='';
				$buddy='';
				$statuspic='<img src="images/icons/online.png" width="7" height="7" alt="online" />';
				
				if(!validate_url(gethomepage($userID))) $hp='';
				else $hp='<a href="'.gethomepage($userID).'" target="_blank"><img src="images/icons/hp.png" border="0" width="14" height="14" alt="'.$_language->module['homepage'].'" /></a>';
				...
				if(isforumadmin($userID) || ismoderator($userID, $board)) {
					if(ismoderator($userID, $board)) {
						...
						$rang='<img src="images/icons/ranks/moderator.png" alt="" />';
					...
					if(isforumadmin($userID)) {
						...
						$rang='<img src="images/icons/ranks/admin.png" alt="" />';
						if(isset($_POST['sticky'])){
							$_sticky = 'checked="checked"';
						}
					}
	...
	foreach($topicID as $id) {
		...
		safe_query("DELETE FROM ".PREFIX."forum_posts WHERE topicID='$id' ");
	}
	header("Location: index.php?site=forum&board=$board");	
}	
elseif($_POST['admaction']=="deletetopics") {
	include("_mysql.php");
	include("_settings.php");
	include('_functions.php');
	$_language->read_module('forum');
	...
	} else header("Location: index.php?site=forum&board=$board");
	if(!isforumadmin($userID) and !ismoderator($userID, $board)) die($_language->module['no_access']);
	$_language->read_module('forum_admin_options', true);
	$pagetitle = PAGETITLE;
	$pagebg = PAGEBG;
	...
	eval ("\$forum_delete_topics = \"".gettemplate("forum_delete_topics")."\";");
	echo $forum_delete_topics;
}
/*END: New adminactions to modify several topics*/	
