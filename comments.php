...
function checkCommentsAllow($type, $parentID){
	...
	$moduls['bl'] = array("user_blog","blogID","comments");
	...
	if(isfeedbackadmin($userID) or iscommentposter($userID,$id)) {
		if(!empty($id)) {
			...
			if(mysql_num_rows($dt)) {
				...
				$poster='<a href="user/'.$ds['userID'].'/">'.getnickname($ds['userID']).'</a>';
...
elseif(isset($_POST['saveeditcomment'])) {
	...
	if(isset($_GET['sorttype']) AND strtoupper($_GET['sorttype']=="DESC")) $sorttype = 'DESC';
	else $sorttype='ASC';
	...
	if($commentspages>1) $page_link = makepagelink("$referer", $commentspage, $commentspages, 'comments');
	else $page_link='';
	...
	if($gesamt) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			if($ds['userID']) {
				$ip='';
				$poster='<a href="user/'.$ds['userID'].'/">'.strip_tags(getnickname($ds['userID'])).'</a>';
				if(isclanmember($ds['userID'])) $member=$_language->module['clanmember_icon'];
				else $member='';
				if(isclanmember($ds['userID'])) $memberborder='border: 2px #8CCBDF solid;';
				else $memberborder='border: 2px #191919 solid;';
				if(isclanmember($ds['userID'])) $memberbg='sm/core/comment-member.png';
				else $memberbg='';
				if(isclanmember($ds['userID'])) $membertag='NC';
				else $membertag='';
				...
				if($loggedin) {
					$quote=str_replace(Array('%nickname%', '%message%'), Array($quotenickname, $quotemessage), '<a href="javascript:AddCode(\'[quote=%nickname%]%message%[/quote]\')"><img src="./sm/forum/quote.png" alt="Quote"/></a>');
				}
				else {
					echo '';
				}
				...
				$avatar='<img src="images/avatars/'.getavatar($ds['userID']).'" align="left" width="90" height="90" alt="Avatar" />';
				...
				$member='';
				$avatar='<img src="images/avatars/noavatar.gif" align="left" width="90" height="90" alt="Avatar" />';
				$country='';
				$pm='';
				$buddy='';
				$statuspic='';
			...
			if(isfeedbackadmin($userID) or iscommentposter($userID,$ds['commentID'])) {
				$edit = '<a href="comment/edit/'.$ds['commentID'].'/ref='.$referer.'" ><img src="./sm/forum/edit.png" alt="Edit Post"/></a>';
			} else $edit='';

			if(isfeedbackadmin($userID)) $actions='<input class="input" type="checkbox" name="commentID[]" value="'.$ds['commentID'].'" style=" margin: 0px;" />';
			else $actions='';

			eval ("\$comments = \"".gettemplate("sm_comments")."\";");
			echo $comments;
			...
			if(isfeedbackadmin($userID)) {
				$submit='<div style="margin: 0px 0px 10px 0px; float: right;"><input type="hidden" name="referer" value="'.$referer.'" />
        <input class="input" type="checkbox" name="ALL" value="ALL" onclick="SelectAll(this.form);" /> '.$_language->module['select_all'].'
        <input type="submit" value="'.$_language->module['delete_selected'].'" /></div>';
			}	else $submit='';
	...
	if($comments_allowed) {
		...
		else echo '<div id="comments_register">
  <div class="attention">Please login to post comments.</div>
</div>';
	}
	else echo '<div id="comments_register">
  <div class="attention">Comments Have Been Disabled!</div>
</div>';
}
