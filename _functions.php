...
function makepagelink($link, $page, $pages, $sub='') {
	$page_link = '<div id="pagelink">';

	if($page != 1) $page_link .= '<a href="'.$link.'page/'.($page-1).'/" class="paginator_selector">Prev</a>';
	if($page >= 6) $page_link .= '';
	if($page+4 >= $pages) $pagex=$pages;
	else $pagex = $page+4;
	for($i=$page-4 ; $i<=$pagex ; $i++) {
		if($i<=0) $i=1;
		if($i==$page) $page_link .= '<span class="paginator_active">'.$i.'</span>';
		else $page_link .= '<a href="'.$link.'page/'.$i.'/" class="paginator_selector">'.$i.'</a>';
	}
	if(($pages-$page) >= 5) $page_link .= '';
	if($page != $pages) $page_link .= '<a href="'.$link.'page/'.($page+1).'/" class="paginator_selector">Next</a>';
	$page_link .= '</div>';
