...
$ergebnis=safe_query("SELECT * FROM ".PREFIX."news WHERE published='1' ".$only." AND intern<=".isclanmember($userID)." AND category='1' ORDER BY date DESC LIMIT 0,". 5);
if(mysql_num_rows($ergebnis)){
	...
	while($ds=mysql_fetch_array($ergebnis)) {
		...
		else $pic = '<img src="images/games/nopic.gif" alt="nopic"/>';
		...
		if(mb_strlen($headlines)>35) {
			$headlines=mb_substr($headlines, 0, 35);
			$headlines.='...';
		}
	...
	if($ds['comments']) {
		...
		eval ("\$sc_headlines = \"".gettemplate("sm_headlines_inner")."\";");
		echo $sc_headlines;
