...
function settitle($string){
	return $string.' | '.$GLOBALS['hp_title'];
}
function settitlehome($string){
...
switch ($GLOBALS['site']) {
	...
	case 'include_home':
		define('PAGETITLE', settitlehome('Esports Club'));
		break;
	...
	case 'streams':
		...
		if($streamID) {
			...
			define('PAGETITLE', settitle($get['title']));
		}
		else {
	...
	case 'videos':
		if(isset($_GET['videoID'])) $videoID = (int)$_GET['videoID'];
		else $videoID = '';
		if($videoID) {
			$get=mysql_fetch_array(safe_query("SELECT title FROM `".PREFIX."videos` WHERE videoID='$videoID'"));
			define('PAGETITLE', settitle($get['title']));
		}
		else {
	...
	case 'lostpassword':
		define('PAGETITLE', settitle('Lost Password'));
		break;
	...
	case 'messenger':
		define('PAGETITLE', settitle('Mail'));
		break;
	
	case 'myprofile':
		define('PAGETITLE', settitle('Settings'));
		break;
	
	case 'news':
		if($action=="archive") {
			define('PAGETITLE', settitle(ucwords($_GET['category']).'&nbsp; &raquo; &nbsp;'.$_language->module['archive']));
		}
		elseif($category=="coverages") {
			define('PAGETITLE', settitle('Coverages'));
		}
		else {
	...
	case 'news_comments':
		...
		$type = $_language->module['news'];
		}
		
		define('PAGETITLE', settitle($type.'&nbsp; &raquo; &nbsp;'.$headline));
		break;
	...
	case 'squads':
		...
		if($action=="show") {
			$get=mysql_fetch_array(safe_query("SELECT name FROM `".PREFIX."squads` WHERE squadID='$squadID'"));
			define('PAGETITLE', settitle('Teams'.'&nbsp; &raquo; &nbsp;'.$get['name']));
		}
		else {
			define('PAGETITLE', settitle('Teams'));
		}
		break;
