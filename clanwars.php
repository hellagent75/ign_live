  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="2">
	...
	if($gesamt) {
		...
		while($ds=mysql_fetch_array($ergebnis)) {
			...
			eval ("\$clanwars_content = \"".gettemplate("sm_inner_clanwars_list")."\";");
...
elseif(empty($_GET['action'])) {
  ...
  if(isclanwaradmin($userID)) $admin='<input type="button" onclick="MM_openBrWindow(\'clanwars.php?action=new\',\'Clanwars\',\'toolbar=no,status=no,scrollbars=yes,width=800,height=600\')" value="'.$_language->module['new_clanwar'].'" />';
	...
	echo''.$seiten.'<span style="float:right;">'.$admin.'</span>';
	...
	if($gesamt) {
     ...
     while($dm=mysql_fetch_array($clanwarslogos)) {
          $opponent_logo = '<img style="float:left; padding-right:7px;" src="images/clanwars/'.$dm['nologo'].'"  alt="" />';
          $ownlogo = $dm['logo'];
          }
     if($ds['banner']) {$opponent_logo='<img style="float:left;  padding-right:7px;" src="images/clanwars/'.$ds['banner'].'" alt="" />'; }
...
if($anzcomments = getanzcwcomments($ds['cwID'])) $details='<a href="match/'.$ds['cwID'].'">';
    ...
    if(isclanwaradmin($userID)) $admdel='<input class="input" type="checkbox" name="ALL" value="ALL" onclick="SelectAll(this.form);" /> '.$_language->module['select_all'].'
