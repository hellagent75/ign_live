...
if(isset($_POST['newreply']) && !isset($_POST['preview'])) {
	...
	header("Location: forum/topic/".$topic."/page/".$page);
	exit();
}
elseif(isset($_POST['editreply']) and (bool)$_POST['editreply']) {
	...
	header("Location: forum/topic/".(int)$_GET['topic']."/page/".(int)$_GET['page'])."";
}
...
function showtopic($topic, $edit, $addreply, $quoteID, $type) {
	...
	$page_link = '';
	if($pages>1) $page_link = makepagelink("forum/topic/$topic/", $page, $pages);
	if($type=="ASC") {
		$sorter='<a href="forum/topic/'.$topic.'/page/'.$page.'">'.$_language->module['sort'].':</a> <img src="images/icons/asc.gif" alt="" />';
	}
	else {
		$sorter='<a href="forum/topic/'.$topic.'/page/'.$page.'">'.$_language->module['sort'].':</a> <img src="images/icons/desc.gif" alt="" />';
	}
	...
	$topicactions='';
	if($loggedin and $writer) $topicactions.='<a class="forum_button" href="index.php?site=forum&amp;addtopic=true&amp;action=newtopic&amp;board='.$dt['boardID'].'">New Topic</a> <a class="forum_button" href="index.php?site=forum_topic&amp;topic='.$topic.'&amp;addreply=true&amp;page='.$pages.'&amp;type='.$type.'">New Reply</a>';
	if($dt['closed']) $closed=$_language->module['closed_image'];
	else $closed='';
	$posttype='topic';
	
	if ($db['boardico']=="") $boardico='<img src="images/forumico/no_icon.jpg"/>';
	else $boardico='<img src="images/forumico/'.$db['boardico'].'.jpg" alt="ICO" />';
	...
	elseif($addreply && !$dt['closed']) {
		if($loggedin and $writer) {
			if(isset($_POST['preview'])) {
				...
				$username='<a href="user/'.$userID.'/">'.getnickname($userID).'</a>';
		...
		elseif($loggedin) {
			echo'<div class="errorbox">You have no permission to view this board<br /></div>';
		}
		else {
			echo '<div id="forum_register"><div class="attention">You must login or <a href="register/">register</a> to use the forum.</div></div>';
		}
	...
	while($dr=mysql_fetch_array($replys)) {
		...
		$username='<a href="user/'.$dr['poster'].'/">'.stripslashes(getnickname($dr['poster'])).'</a>';
		...
		if(!$dt['closed']) $quote='<a href="index.php?site=forum_topic&amp;addreply=true&amp;board='.$dt['boardID'].'&amp;topic='.$topic.'&amp;quoteID='.$dr['postID'].'&amp;page='.$page.'&amp;type='.$type.'"><img src="sm/forum/quote.png" border="0" alt="'.$_language->module['quote'].'" /></a>';
		else $quote = "";
		...
		$actions='';
		if(($userID == $dr['poster'] OR isforumadmin($userID) OR ismoderator($userID,$dt['boardID']))&& !$dt['closed']) $actions=' <a href="index.php?site=forum_topic&amp;topic='.$topic.'&amp;edit=true&amp;id='.$dr['postID'].'&amp;page='.$page.'"><img src="sm/forum/edit.png" border="0" alt="'.$_language->module['edit'].'" /></a> ';
	...
	echo'<div align="right">'.$adminactions.'</div>';
