			...
			if($getimg[0] < 199 && $getimg[1] < 81) {
				...
				if($pic != "") {
					...
					if(unlink($filepath.$banner['name'].".tmp")) {
						...
						die('<b>'.$error.'</b><br /><br /><a href="admincenter.php?site=featured_items&amp;action=edit&amp;partnerID='.$id.'">&laquo; '.$_language->module['back'].'</a>');
					} else {
						$error = $_language->module['format_incorrect'];
						die('<b>'.$error.'</b><br /><br /><a href="admincenter.php?site=featured_items&amp;action=edit&amp;partnerID='.$id.'">&laquo; '.$_language->module['back'].'</a>');
					}
...
if($action=="add") {
	...
	echo'<h1>&curren; <a href="admincenter.php?site=featured_items" class="white">'.$_language->module['partners'].'</a> &raquo; '.$_language->module['add_partner'].'</h1>';

	echo'<form method="post" action="admincenter.php?site=featured_items" enctype="multipart/form-data">
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
    ...
    <tr>
      ...
      <td><input name="banner" type="file" size="40" /> <small>'.$_language->module['max_198x80'].'</small></td>
    </tr>
    <tr>
...
elseif($action=="edit") {
  ...
  echo'<h1>&curren; <a href="admincenter.php?site=featured_items" class="white">'.$_language->module['partners'].'</a> &raquo; '.$_language->module['edit_partner'].'</h1>';
  ...
  echo'<input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=featured_items&amp;action=add\');return document.MM_returnValue" value="'.$_language->module['new_partner'].'" /><br /><br />';

	echo'<form method="post" action="admincenter.php?site=featured_items">
  <table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#DDDDDD">
    ...
    if($days) $perday=round($db['hits']/$days,2);
      ...
      <td class="'.$td.'" align="center"><input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=featured_items&amp;action=edit&amp;partnerID='.$db['partnerID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
      <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'admincenter.php?site=featured_items&amp;delete=true&amp;partnerID='.$db['partnerID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['delete'].'" /></td>
