...
elseif($part=="gallerys") {
	...
	if(isset($_POST['save'])) {
		...
		if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
			if(checkforempty(Array('name'))) {
				safe_query("INSERT INTO ".PREFIX."gallery ( name, albuminfo, date, groupID ) values( '".$_POST['name']."', '".$_POST['albuminfo']."', '".time()."', '".$_POST['group']."' ) ");
				$id = mysql_insert_id();
			} else echo $_language->module['information_incomplete'];
		} else echo $_language->module['transaction_invalid'];
	}
	...
	elseif(isset($_POST['saveedit'])) {
		...
		if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
			if(checkforempty(Array('name'))) {
				...
				safe_query("UPDATE ".PREFIX."gallery SET name='".$_POST['name']."', albuminfo='".$_POST['albuminfo']."', groupID='".$_POST['group']."' WHERE galleryID='".$_POST['galleryID']."'");
			} else echo $_language->module['information_incomplete'];
		} else echo $_language->module['transaction_invalid'];
	}
    ...
    $ergebnis=safe_query("SELECT * FROM ".PREFIX."gallery_groups");
	    ...
	    <table width="100%" border="0" cellspacing="1" cellpadding="3">
		  ...
		  <tr>
	        <td width="15%"><b>Info</b></td>
	        <td width="85%"><input type="text" name="albuminfo" size="60" /></td>
	      </tr>
	      <tr>
    ...
    <table width="100%" border="0" cellspacing="1" cellpadding="3">
	  ...
	  <tr>
        <td width="15%"><b>Info</b></td>
        <td width="85%"><input type="text" name="albuminfo" value="'.getinput($ds['albuminfo']).'" /></td>
      </tr>';
